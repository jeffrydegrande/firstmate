...
A decision is simply a task held for the user: `tasks-axi hold <id> --reason "<reason>" --kind captain`, with `--until <date>` when the user defers it.
When a main-side thread such as a pending user decision or relay reminder is worth durable tracking, file it as its own work item and hold it the same way.
