...
The one primitive is an ordinary backlog task held for the user (`tasks-axi hold <id> --kind captain`), its identity is the task id, and `bin/fm-captain-hold.sh` owns the deterministic mechanics this policy relies on.
...
When the user says "later", that is an answer too: re-hold with `tasks-axi hold <id> ... --until <date>` so the item leaves the live Needs You and resurfaces on its date, instead of leaving a live-looking card or fabricating a closure.
