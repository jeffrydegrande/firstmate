...
When any diagnostic needs user attention, report the plain consequence and requested action using `AGENTS.md` section 9's user-facing translation contract; do not name the diagnostic label unless the user needs to paste it into a command or issue.
...
- `MISSING_MANUAL: <tool> (instructions: <url>)` - tell the user why the tool is required and give them the printed instructions URL, but do not pass the tool to `bin/fm-bootstrap.sh install`; wait for the user to complete the manual installation, then rerun session start to confirm the dependency is present.
...
- `NEEDS_GH_AUTH` - ask the user to run `! gh auth login` (interactive; you cannot run it for them).
  This probe now arrives from the deferred network stage, so it is also how an unreachable network shows up: `gh` cannot validate its token offline and reports the same failure. Confirm reachability before asking the user to re-authenticate a credential that may be fine.
