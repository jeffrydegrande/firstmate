...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITHOUT one** (e.g. pi): run `bin/fm-afk-launch.sh start`. It is
     ...
     shrinks the user's pane (docs/herdr-backend.md "Away-mode supervisor
     support").
...
4. **Acknowledge** in `AGENTS.md` section 9 language: "User, away mode is active; I will batch routine updates and surface only decisions, failures, credentials, or review-ready work until you return."

## How to exit afk

No `/back` is needed. The first genuine message is the return signal:

- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the user is back.
  Run `bin/fm-afk-return.sh` before acting on the message that brought the user back.
  ...
  Do not answer a Bearings request or perform any other ordinary user work until the check exits successfully.
...
A PR ready for merge keeps the merge authority from `AGENTS.md` section 7, and a needs-decision finding keeps the `ask-user-authority` policy; anything requiring the user still waits for the user's explicit word.
...
User-relevant events, plus a bounded recheck of a declared wait that is still declared, escalate to firstmate's context as one pre-read, single-line, batched digest.
The user-relevant verb set, declared-wait vocabulary, status-span classifier, and presentation-marker contract live in shared `bin/fm-classify-lib.sh`, while each supervisor owns its routing and fleet scan as a consumer of that policy.
While `state/.afk` exists the daemon owns the watcher, so the watcher reverts to one-shot and lets the daemon do the triage - the two never run their triage at the same time.

Classify each wake this way:

- `signal` whose newly classified status span contains user-relevant events -> escalate every event in source order.
  ...
  Other signals with no user-relevant event in the span -> self-handle.
- `signal` or `stale` whose latest status declares a wait, either a `paused:` external wait or a verified `user-held` transfer, tracks the pause rather than a wedge whether its pane reads idle or busy.
  An unreported user-relevant event in the newly classified span still escalates immediately while the current declaration independently keeps the pause cadence.
  ...
  That recheck names which human the wait is on: the external dependency for `paused:`, and the user themself for a `user-held` transfer, who can answer the held decision or release the hold.
- `check` -> always escalate. Check scripts print only when firstmate should wake.
- `stale` with a terminal status or bare legacy user-relevant line -> escalate.
...
- `heartbeat` -> self-handle.
  The daemon runs its own cheap bash fleet scan every `FM_HEARTBEAT_SCAN_SECS` (default 300s) as the catch-all for user-relevant events still unread by the per-wake classifier.
...
the operational prefix lets firstmate distinguish it from a real user message.
