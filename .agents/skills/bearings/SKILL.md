...
1. **Gather live fleet state with one deterministic command.**
   ...
   A decision is simply a task held for the user (`captain-hold-lifecycle`); every due, unblocked user-held task appears under `decisions_open`, whatever its kind.
   A user hold deferred by date sits under `gates` with its `until <date>:` reason until it is due, and a hold whose reason or body carries an explicit deferred/superseded marker is suppressed from the default view with an `omitted` disclosure.
...
4. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Needs You** - every open decision summarized with its options from the structured decision record, plus each PR ready to merge and each needed credential or login, every PR with the full `https://...` URL, never a bare `#number`.
...
1. **Needs You** - ONLY items that need the user's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only the user can clear.
...
4. **Charted Next** - queued or gated work waiting on the fleet or a date, plus action-free fleet-integrity warnings, never on the user.
...
- The four buckets are mutually exclusive per item: needs-your-action is Needs You, done is Recently Landed, self-progressing is Underway, and not-yet-started work or an action-free fleet-integrity warning is Charted Next.
- A secondmate home can contribute to more than one section at once. Each active child is an Underway row regardless of the home-level `bearings_state`, while that same home's due user hold is Needs You and its queued or external holds stay Charted Next. Do not hide active children because the home also has an open user hold.
