...
description: >-
  ...
  Use when the user invokes /bearings or asks for a bearings report, morning brief, status report, catch-up, "where did I leave off", or "what's in the works".
...
Generate a complete current snapshot from the fleet's current state, so the user can resume in one read after a break, a night, or a context reset.
...
- When the user asks to include PRs, pass the snapshot command's live-PR opt-in.
...
1. **Gather live fleet state with one deterministic command.**
   ...
   Only pass `--include-prs` when the user asks for live GitHub PR enrichment.
   ...
   A decision is simply a task held for the user (`captain-hold-lifecycle`), whatever its kind.
   The canonical snapshot assigns every user hold exactly one bucket from structured fields only: `blocked` when any blocker is unresolved, else `dated` while `hold_until` is in the future, else `aged` when an undated hold has reached the configured age threshold, else `live`.
   ...
   Use `--all-decisions` to reveal every user hold available within the bounded snapshot and remove each revealed gate from Charted Next so the buckets remain exclusive.
...
4. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Charted Next** - queued or gated work, including deferred or aged user-hold safety gates and any main-inventory integrity warning, with each item's blocker, date, age, or integrity reason.
...
- A Needs You decision key is the user-held TASK ID from `decisions_open` (legacy `<origin>-decision-<key>` rows are already task ids); a merge card's key is `merge.<task-id>`; the Charted Next dispatch picker's key is `dispatch.charted`.
- Compose exactly one decision card per user-held task id. When one task carries multiple questions, consolidate all of them and their options into that card; never emit duplicate cards with the same task-id key.
...
- When the card's task is a user-gated WORK item (the answer should free it to proceed rather than complete it), set the card's `close: "release"` so the answer lifts the hold instead of closing the task; question-shaped items omit it.
...
Decision answers need no routing from you: the runner feeds the board's binding into `bin/fm-captain-hold.sh`'s one keyed-answer intake, which closes or releases each answered user-held task at answer time; reconcile any `skipped:` key yourself with a direct `answer`, and when the user's answer is "later", record it as a deferral with `bin/fm-captain-hold.sh hold <id> --reason "<reason>" --until <date>` instead of a closure.
Route the non-decision keys yourself:

- `merge.<task-id>` is the user's explicit merge order; follow the merge ruling below.
- `dispatch.charted` carries comma-separated task ids the user picked to start now; verify each id against the current backlog - still queued, blocker and time gate actually clear - then dispatch through the normal lifecycle, and report any id that no longer qualifies instead of forcing it.
...
### The merge-click ruling (user-decided)

A board "Merge now" answer IS the user's explicit merge word for that one exact PR; ask no second confirmation.
...
Only the exact answer value `merge` authorizes a merge; an answer carrying a freeform note is the user's instruction text to read and act on with judgment, never an auto-merge.
...
4. **Charted Next** - queued or gated work waiting on the fleet or a date, deferred or aged user-hold safety gates, plus action-free fleet-integrity warnings.
...
- A user hold appears in exactly one decision bucket: an unsuppressed live hold is in Needs You, while a blocked, dated, or aged hold is in Charted Next; `--all-decisions` moves the latter into Needs You and removes its gate.
- Underway independently reports active work, so an actively worked user-held task may appear there plus its one decision bucket.
- A secondmate home can contribute to more than one section at once. Each active child is an Underway row regardless of the home-level `bearings_state`, while that same home's live user hold is Needs You and its queued or external holds stay Charted Next. Do not hide active children because the home also has an open user hold.
...
- A secondmate's own home-level row is not an Underway unit: `externally_held` belongs in Charted Next, and `unknown` belongs there as an unavailable-state gate unless its reason requires the user's action.
...
- Include the required direct address to the user inside one item or empty-state sentence.
...
- The optional file-mode report is a private, user-facing internal artifact that lives in gitignored `data/`, so unlike normal user chat it MAY reference task ids, PR URLs, and repo names.
- The user works with those directly and needs them to resume; keep the report organized and scannable, not a raw dump.
...
On a later board wake, this read-only invocation rule yields to "Handling a board wake" and its guarded authority for user-selected dispatches and merges.
