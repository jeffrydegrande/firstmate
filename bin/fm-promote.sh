...
# default-branch base, the ship branch, and - rendered from
...
# --branch <name> sets the ship branch used in the Setup step and the Definition
# of done, and is recorded as branch= in the task meta so the landing and review
# helpers resolve the real branch. Without --branch, the deprecated fm/<task-id>
# default is used with a one-line stderr warning.
...
ID=${POS[0]}
# Ship branch the promoted worker creates. Firstmate sets it at intake; without
# --branch the deprecated fm/<id> default is used with a loud warning.
