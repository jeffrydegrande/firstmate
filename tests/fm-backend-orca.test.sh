...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  assert_contains "$(cat "$log")" "CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"theme\":\"dark-daltonized\"" \
    "spawn did not send the selected crewmate harness launch command (with the CREWMATE theme) through Orca"
  rm -rf "/tmp/fm-$id"
