...
EOF
  ...
  # The removed tool must be one that BASE_PATH can never provide, so the
  # MISSING line is deterministic on any host. A generic tool like node leaks
  # from a system /usr/bin, so use a firstmate-specific tool that only ever
  # lives in the fake bin.
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Remove a firstmate-specific tool, never a generic one like node: node can
  # leak from a system /usr/bin, so its MISSING line is not deterministic on
  # every host. chrome-devtools-axi only ever lives in the fake bin.
  rm -f "$fakebin/chrome-devtools-axi"
