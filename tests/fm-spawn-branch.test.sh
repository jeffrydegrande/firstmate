...
test_spawn_records_brief_branch_without_flag() {
  ...
  read_case "$rec"
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN" spawn-br-a1 "$PROJ_DIR" --mode local-only --yolo off)
  status=$?
...
test_spawn_flag_records_branch() {
  ...
  read_case "$rec"
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN" spawn-br-b1 "$PROJ_DIR" \
...
test_spawn_records_default_fm_branch() {
  ...
  read_case "$rec"
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN" spawn-br-c1 "$PROJ_DIR" --mode local-only --yolo off)
  status=$?
...
test_spawn_branch_mismatch_is_refused() {
  ...
  read_case "$rec"
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN" spawn-br-d1 "$PROJ_DIR" \
...
test_spawn_branch_refused_on_scout() {
  ...
  read_case "$rec"
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN" spawn-br-e1 "$PROJ_DIR" --scout --branch foo)
  status=$?
